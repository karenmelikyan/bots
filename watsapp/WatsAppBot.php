...
class WatsAppBot
{
            ...
            foreach($decoded['messages'] as $message)
            {
                ...
                if(!$message['fromMe'])
                {
                    ...
                    switch(mb_strtolower($text[0], 'UTF-8'))
                    {
                        ...
                        break;
                        case 'chatid': $this->showChatId($message['chatId']); 
                        break;
                        case 'time': $this->time($message['chatId']); 
                        break;
                        case 'me':  $this->me($message['chatId'],$message['senderName']); 
                        break;
                        case 'file': $this->file($message['chatId'],$text[1]); 
                        break;
                        case 'ptt': $this->ptt($message['chatId']); 
                        break;
                        case 'geo': $this->geo($message['chatId']); 
                        break;
                        case 'group': $this->group($message['author']); 
                        break;
                        default: $this->welcome($message['chatId'], true); 
                        break;
                    }
                }
            }
    ...
    public function welcome(string $chatId, bool $noWelcome = false): void
    {
    ...
    //@param $chatId [string] [required] - the ID of chat where we send a message
    public function showChatId(string $chatId): void
    {
    ...
    //@param $chatId [string] [required] - the ID of chat where we send a message
    public function time(string $chatId): void
    {
    ...
    public function me(string $chatId, sring $name): void
    {
    ...
    public function file(string $chatId, string $format): void
    {
        $availableFiles = [
            'doc' => 'document.doc',
            'gif' => 'gifka.gif',
    ...
    //@param $chatId [string] [required] - the ID of chat where we send a message
    public function ptt(string $chatId): void
    {
        $data = [
    ...
    //@param $chatId [string] [required] - the ID of chat where we send a message
    public function geo(string $chatId): void
    {
        $data = [
            'lat' => 51.51916,
            'lng' => -0.139214,
    ...
    public function group(string $author): void
    {
     ...
     * 
     */
    public function sendRequest(string $method, string $data): void
    {
